...
def _plain_room(room, room_height, wall, floor, torch):
    ...
    return [
        ...
        fill_region([x0, 1, z0], [x1, room_height + 1, z0 + 1], block(wall)),
        fill_region([x0, 1, z1 - 1], [x1, room_height + 1, z1], block(wall)),
        fill_region([x0, 1, z0 + 1], [x0 + 1, room_height + 1, z1 - 1], block(wall)),
        fill_region([x1 - 1, 1, z0 + 1], [x1, room_height + 1, z1 - 1], block(wall)),
        fill_region([x0, room_height + 1, z0], [x1, room_height + 2, z1], block(wall)),
...
def BspDungeon(
    ...
    parts = []
    for room in rooms:
        parts.extend(_plain_room(room, room_height, wall, floor, torch))
    ...
    if surface_entrance:
        ...
        parts.extend([
            ...
            fill_region([hx0, surface_level + 1, 0], [hx1, surface_level + 4, 1], block(wall)),
            fill_region([hx0, surface_level + 1, 4], [hx1, surface_level + 4, 5], block(wall)),
            fill_region([hx0, surface_level + 1, 1], [hx0 + 1, surface_level + 4, 4], block(wall)),
            fill_region([hx1 - 1, surface_level + 1, 1], [hx1, surface_level + 4, 4], block(wall)),
    ...
    for cell in ordered_corridor_cells:
        x, z = cell[0], cell[1]
        parts.extend([
            place_block([x, 0, z], block(floor)),
            place_block([x, room_height + 1, z], block(wall)),
            carve_region([x, 1, z], [x + 1, room_height + 1, z + 1]),
        ])
        if _key(x - 1, z) not in corridor_cells and _key(x - 1, z) not in stair_cells:
            parts.append(fill_region([x - 1, 1, z], [x, room_height + 2, z + 1], block(wall)))
        if _key(x + 1, z) not in corridor_cells and _key(x + 1, z) not in stair_cells:
            parts.append(fill_region([x + 1, 1, z], [x + 2, room_height + 2, z + 1], block(wall)))
